/*
 * This file was generated by the Shade SDK generator and does not guarantee 100% accuracy of the information.
 * This file may be used for educational purposes only.
 */

/*
 * Link to the SDK generator: https://github.com/arisuwine/shadegenerator
 * Link to the SDK generator developer: https://github.com/arisuwine
 */

#pragma once
#include <cstdint>
#include <memory>
#include <utility>

#include "shade/sdk/types.hpp"

#include "shade/sdk/client/CCitadelModifier.hpp"
#include "shade/sdk/client/ParticleIndex_t.hpp"

namespace shade {
    namespace sdk {
        namespace client {
            /* Class Parameters
             * Size: 0x3f8
             * Alignment: 0xff
             * Has VTable
             * Module Local Type Scope
             */
            #pragma pack(push, 1)
            class CCitadel_Modifier_MageWalk : public shade::sdk::client::CCitadelModifier {
            public:
                bool m_bIsFakeout; // 0x00c0, 0x1 bytes
                bool m_bTeleported; // 0x00c1, 0x1 bytes
                std::uint8_t pad_00c2[0x2]; // 0x00c2, 0x2 bytes
                shade::sdk::client::ParticleIndex_t m_particleStart; // 0x00c4, 0x4 bytes
                shade::sdk::client::ParticleIndex_t m_particleEnd; // 0x00c8, 0x4 bytes
                shade::sdk::client::ParticleIndex_t m_particleTrail; // 0x00cc, 0x4 bytes
                Vector m_vecEndLocation; // 0x00d0, 0xc bytes
                Vector m_vecStartPosition; // 0x00dc, 0xc bytes
                Vector m_vecEndLocationCaster; // 0x00e8, 0xc bytes
                std::uint8_t pad_00f4[0x304]; // 0x00f4, 0x304 bytes
            };
            #pragma pack(pop)

            // No unique data map fields

            static_assert(sizeof(CCitadel_Modifier_MageWalk) == 0x3F8, "CCitadel_Modifier_MageWalk size mismatch");
        }
    }
}
