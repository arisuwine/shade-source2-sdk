/*
 * This file was generated by the Shade SDK generator and does not guarantee 100% accuracy of the information.
 * This file may be used for educational purposes only.
 */

/*
 * Link to the SDK generator: https://github.com/arisuwine/shadegenerator
 * Link to the SDK generator developer: https://github.com/arisuwine
 */

#pragma once
#include <cstdint>
#include <memory>
#include <utility>

#include "shade/sdk/types.hpp"

#include "shade/sdk/client/AuraCenterType_t.hpp"
#include "shade/sdk/client/AuraShapeType_t.hpp"
#include "shade/sdk/client/CModifierLevelFloat.hpp"
#include "shade/sdk/server/CCitadelModifierVData.hpp"

namespace shade {
    namespace sdk {
        namespace server {
            class CBaseModifier;
        }
    }
}

namespace shade {
    namespace sdk {
        namespace server {
            /* Class Parameters
             * Size: 0x790
             * Alignment: 0x8
             * Has VTable
             * Construct Allowed
             * Module Local Type Scope
             */
            #pragma pack(push, 1)
            class CModifierVData_BaseAura : public shade::sdk::server::CCitadelModifierVData {
            public:
                shade::sdk::client::AuraShapeType_t m_nAuraShapeType; // 0x0750, 0x4 bytes
                shade::sdk::client::AuraCenterType_t m_nCenterType; // 0x0754, 0x4 bytes
                shade::sdk::client::CModifierLevelFloat m_flAuraRadius; // 0x0758, 0x10 bytes
                shade::sdk::client::CModifierLevelFloat m_flAuraEntityBoundsScale; // 0x0768, 0x10 bytes
                std::int32_t m_nAmbientParticleRadiusControlPoint; // 0x0778, 0x4 bytes
                std::uint8_t pad_077c[0x4]; // 0x077c, 0x4 bytes
                CEmbeddedSubclass<shade::sdk::server::CBaseModifier> m_modifierProvidedByAura; // 0x0780, 0x10 bytes
            };
            #pragma pack(pop)

            // No unique data map fields

            static_assert(sizeof(CModifierVData_BaseAura) == 0x790, "CModifierVData_BaseAura size mismatch");
        }
    }
}
